...
public class JobifyDatabase {
   ...
   public JobifyDatabase(String file) {
	...
	/*
	 * This method has no parameter and returns employer object that not null in the
	 * list of all the employers This method is helpful as it prevents error when
	 * fields and one of the field is null. Return a new employer array filled with
	 * non-null employer objects
	 */
   public Employer[] getExistingEmployer() {
	...
	// *****File input
	/*
	 * This method uses the knowledge of file input that we have learned in class
   ...
   public void loadFromFile() {
	...
	// *****File output
	/*
	 * This method uses the knowledge of file output that we have learned in class
   ...
   public void saveToFile() {
	...
	// *****File output
	/*
	 * This method takes in a job array and writes the information to a file
   ...
   public void postJobPrompt() {
      ...
      System.out.println("What location is the job(1. A1, 2. B1, 3. C1, 4. A2, 5. B2, 6. C2)");
      ...
      if (type == 1) {
         ...
         // Find the first job object that is null and saves the new job into the null job with found index
         ...
         if (found) {
            jobs[currentUser][index] = new FullTimeJob(index, title, location, industry, companySize, eduBackground, workExp,
               	gender, monthlyEarnings, benefits, sickDays, breaks);
            numJobs++;
         ...
         if (found) {
            jobs[currentUser][index] = new PartTimeJob(index, title, location, industry, companySize, eduBackground, workExp,
               	gender, hourlyEarnings, minWorkHourPerWeek);
            numJobs++;
            employerAccount[currentUser].postJob(jobs[currentUser][index],index);
         } else {
            System.out.println("You can no longer post job, all filled");
	...
	// This method is used to create a Job object from a file
         ...
         if (in.readLine().compareTo("1") == 0) {
            ...
            if (found) {
               jobs[currentUser][index] = new FullTimeJob(index, in.readLine(), Integer.parseInt(in.readLine()),
                     	...
                     	Integer.parseInt(in.readLine()), Integer.parseInt(in.readLine()),
                     	in.readLine().charAt(0), Double.parseDouble(in.readLine()), in.readLine().charAt(0),
                     	in.readLine().charAt(0), in.readLine().charAt(0));
               System.out.println(jobs[currentUser][index]);
               numJobs++;
               employerAccount[currentUser].postJob(jobs[currentUser][index],index);
            } else {
               System.out.println("You can no longer post job, all filled");
            ...
            if (found) {
               jobs[currentUser][index] = new PartTimeJob(index, in.readLine(), Integer.parseInt(in.readLine()),
                     	Integer.parseInt(in.readLine()), Integer.parseInt(in.readLine()),
                     	Integer.parseInt(in.readLine()), Integer.parseInt(in.readLine()),
                     	in.readLine().charAt(0), Double.parseDouble(in.readLine()),
   ...
   public void addAccount() {
      ...
      // Creates a new Employer object in the previously aquired slot
         if (found) {
            ...
            employerAccount[numAccounts] = new Employer(username, password, firstName, lastName, companyName, phoneNum);
            numAccounts++;
         }
      } catch(Exception e){
   ...
   public Job[][] getJobs(){
	...
	// This method is used to check if the new employer's username is already taken, then return a boolean
   // the parameter is the username prompt from the user
   ...
   public void deleteJob() {
	...
	/*
	 * This method prompts the employee user to enter their information. Purpose: By
	 * creating an object, the employee's data can be stored and be used for their
   ...
   public Employer[] getEmployerAccount() {
	...
	// ***** Sorting
	/*
	 * sort the given job array from high wage to low wage Uses sorting algorithm(s)
	 * to compare wages from high to low and reorders the jobs to a temporary job
	 * array The sorting algorithm used in this method is the bubble sort
	 * algorithm
	 */
   public Job[] sortByWage(Job[] list) {
      ...
      for(int i = job.length-1; i > 0 && !sorted; i--){
         ...
         for(int j = 0; j < i; j++){
            ...
            if ((job[j] instanceof FullTimeJob) && (job[j + 1] instanceof FullTimeJob)&& ((FullTimeJob) job[j]).getMonthlyEarning() < ((FullTimeJob) job[j + 1]).getMonthlyEarning()){
               temp = job[j];
               job[j] = job[j+1];
               job[j+1]= temp;
               sorted = false;
      ...
      for(int i = job.length-1; i > 0 && !sorted; i--){
         sorted = true;
         for(int j = 0; j < i; j++){
            // check for if the first object job[j+1]is full time and job[j] is part time and if true, swap as the heirchy is incorrect 
      ...
      for(int i = job.length-1; i > 0 && !sorted; i--){
         ...
         for(int j = 0; j < i; j++){
            ...
            //and look at the monthly earning for each and swap if possible
            if ( (job[j] instanceof PartTimeJob)&& (job[j + 1] instanceof PartTimeJob)&& ((PartTimeJob) job[j]).getHourlyEarning() < ((PartTimeJob) job[j +1]).getHourlyEarning()){
               temp = job[j];
               job[j] = job[j+1];
               job[j+1]= temp;
               sorted = false;
	...
	// This method takes in a string username and string password which are
	// previously prompted from the user
	// Then returns a employer object that matches the username and password
   ...
   public Employee getEmployee(){
	...
	// ***** Sorting
	/*
	 * sort the given job array from high company size to low company size Uses
	 * sorting algorithm(s) to compare company size from high to low and reorders
	 * the jobs to a temporary job array The sorting algorithm used in this method
   ...
   public Job[] sortByCompanySize(Job[] list) {
	...
	// *****Searching + Recursion
	// The method that returns a list of job that matches the company keyword
	// which was prompted from the user in the method
	// Purpose of this method is not to make the user enter too much but not too
   ...
   public Job[] searchByCompany() {
      ...
      for (int i = 0; i < numAccounts; i++) {
         ...
         // use the boolean returned to determine the size of the array that keep track of the employer account matched
            numMatch++;
         }
      }
      // first use array to keep track of the employer index in which that employer's company name matches the keyword
      int[] matchNumber = new int[numMatch];
      numMatch = 0;
      for (int i = 0; i < numAccounts; i++) {
         if (equalSectionIgnoreCase("", input, employerAccount[i].getCompanyName(), 0)) {
            // use the boolean returned to determine the index in the array of employer that match the company name
	...
	// *****Recursion
	// parameter: Takes in a empty string(longerString-determined in the recursive
	...
	// value of 0.
	// Purpose: check if the smaller string is a part of the larger string.
   ...
   public Job[] searchByTitle() {
	...
	/*
	 * This method searches the database and returns a list of jobs that match the
	 * full time employee's specified demands
	 */
   public Job[] searchFullTimeDemand() {
	...
	/*
	 * This method searches the database and returns a list of jobs that match the
	 * part time employee's specified demands
	 */
   public Job[] searchPartTimeDemand() {
      ...
      industry = Integer.parseInt(input);
   	...
   	// Verifies the input and continues prompting if it is invalid
      while (!checker) {
         System.out.print("Minimum Company Size: ");
         input = sc.nextLine();
         try {
            companySize = Integer.parseInt(input);
            checker = true;
	...
	// display job - takes in a 2-D array and print it to the screen
   public void displayJob(Job[][] joblist) {
	...
	// display job - takes in a 1-D array and print it to the screen
   public void displayJob(Job[] joblist) {
      for (int i = 0; i < joblist.length; i++) {
         if (joblist[i]!= null){
   ...
   public Job[] getExistingJob(Job[] list) {
	...
	// This method returns a 1-D array that contains all full time jobs
   ...
   //This method return 1-d version of the jobs 2d array
   public Job[] get1dJob(){
      int max = 100;
      int count = 0;
	...
	// This method returns a 1-D array that contains all part time jobs
   ...
   public boolean login(String username, String password) {
      ...
      for (int i = 0; i < employerAccount.length && !found; i++) {
         if (employerAccount[i].matchLogin(username, password)) {
            currentUser = i;
	...
	// This method returns the index of the employer who posts the job
   public int matchJob(Job job) {
	...
	// Allows the user to selece a job from the listings
